...
@Component
public class InvoiceSearchSpecificaton {
	
	
	
	public  Specification<Invoice> findByCriteria(final CurrentInvoicesBean filter) {
		return new Specification<Invoice>() {
			...
			public Predicate toPredicate(Root<Invoice> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (!StringUtils.isEmpty(filter.getInvoiceNumSearch())) {
					predicates.add(cb.like(cb.lower(root.get("invoiceNum")),
							"%" + filter.getInvoiceNumSearch().trim().toLowerCase() + "%"));
